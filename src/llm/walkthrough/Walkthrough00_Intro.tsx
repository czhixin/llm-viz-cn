...
export function walkthroughIntro(args: IWalkthroughArgs) {
    ...
    let c0 = commentary(wt, null, 0)`欢迎来到GPT大语言模型的演练！在这里，我们将探索仅为85,000个参数的 _nano-gpt_ 模型。

它的目标很简单：获取六个字母的序列： ${embed(ExampleInputOutput)}
并按字母顺序对它们进行排序，即 "ABBBCC".`;
    ...
    let tokenIdxStr = c_str('_标记索引_', 0, DimStyle.TokenIdx);

    commentary(wt, t6)`我们称这些字母为 "标记"（${tokenStr}），模型的不同标记集合构成了它的_词汇表_：${embed(TokenVocab)}

    在这个表格中，每个标记都有一个编号，即${tokenIdxStr}。现在，我们可以将这一串数字输入模型：${embed(ExampleTokenValues)}\n`;
    breakAfter();
    ...
    let c5 = commentary(wt)`在3D视图中，每个绿色单元格代表一个正在处理的数字，每个蓝色单元格代表一个权重。${embed(GreenBlueCells)}
    序列中的每个数字首先被转化为48个元素向量（根据本特定模型选择的大小）。这就是所谓的_嵌入_(_embedding_)。`;
    breakAfter(c5);
    ...
    breakAfter();
    commentary(wt)`然后，嵌入(embedding)将穿过模型，经过一系列称为转换器(transformers)的层，最后到达底层。`;
    breakAfter();
    ...
    commentary(wt)`那么输出是什么呢？对序列中下一个标记(token)的预测。因此，在第6个条目中，我们可以得到下一个标记将是 "A"、"B "或 "C "的概率。`

    commentary(wt)`在这种情况下，模型非常确定会是 "A"。现在，我们可以将这一预测反馈到模型的顶层，并重复整个过程。`;
